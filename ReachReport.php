...
try {
  ...
  $reportQuery->dimensions = array('ORDER_ID');
  $reportQuery->columns = array('REACH_FREQUENCY', 'REACH');
  ...
  $filePath = sprintf('%s.csv.gz', 'reach-report-' . date('m-d-Y_H-i'));
  ...
  $shell = shell_exec("/usr/local/bin/gsutil cp reach-report*.csv gs://api-hub-output/dfp-reports/reach-report/");
  //foreach($shell as $id => $sh) {
  //  printf($sh);
  //}

  printf("Deleting downloaded files\n");
  shell_exec("rm reach-report*.csv & rm reach-report*.csv.gz");

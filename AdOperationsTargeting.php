...
try {
  ...
  $reportQuery->dimensions = array('LINE_ITEM_ID', 'TARGETING'); //, 'CREATIVE_SIZE'); //, 'AD_UNIT_ID', 'AD_UNIT_NAME'); //, 'TARGETING', 'CUSTOM_TARGETING_VALUE_ID');
  //$reportQuery->dimensionAttributes = array('LINE_ITEM_START_DATE_TIME', 'LINE_ITEM_END_DATE_TIME', 'LINE_ITEM_GOAL_QUANTITY', 'LINE_ITEM_PRIORITY', 'LINE_ITEM_LIFETIME_IMPRESSIONS', 'LINE_ITEM_LIFETIME_CLICKS');
  ...
  $filePath = sprintf('%s.csv.gz', 'targeting-' . date('m-d-Y_H-i'));
  ...
  $shell = shell_exec("/usr/local/bin/gsutil cp targeting*.csv gs://api-hub-output/dfp-reports/ad-operations/targeting/");
  //foreach($shell as $id => $sh) {
  //  printf($sh);
  //}

  printf("Deleting downloaded files\n");
  shell_exec("rm targeting*.csv & rm targeting*.csv.gz");
